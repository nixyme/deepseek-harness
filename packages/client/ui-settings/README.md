...
- **Ordinary non-loopback pages get no durable settings** — Host persistence is enabled for loopback or mandatory-principal remote pages only. Other remote pages keep Host persistence disabled, so a scope starts `unavailable` and never crosses the wire; every row it backs is inert.
